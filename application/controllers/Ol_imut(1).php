...
//=================================================== QC
  ...
  function q_control($mode='view'){
    ...
    if($mode=='view'){
      ...
      $data['cmd_status'] = $this->m_rancak->cmd_status();
      $data['cmd_ya_tidak'] = $this->m_rancak->cmd_ya_tidak();
      $kone = array('status_eq_detil'=>1,'status_equipment'=>1,'id_unit'=>$this->session->unit,'jenis_equipment'=>2);
      $data['equipment'] = $this->m_ol_rancak->ambil_equipment_imut($kone);
      if($mode=='tambah'){
        $data['page'] =  $data['page']."_tambah";
        $data['tgl_eq_imut']  = set_value('tgl_eq_imut',date('d-m-Y'));     
    ...
    if($mode=='view'){
      ...
      if(empty($data['id3'])){
        if($this->session->has_userdata('range_report_imutu')){
          $data['id3'] = $this->session->range_report_imutu;
        }else{
          $data['id3'] = 1;
        }
      }
      ...
      if($action == 'BtnProses') {
        $first_date = $this->input->post("id");
        $last_date = $this->input->post("id2");
        $range_report_imutu = $this->input->post("id3");
        $data_user_level = array(
        ...
        $this->session->set_userdata($data_user_level); 
        redirect(base_url('ol_imut/'.$data['page'].'/view/'.$first_date.'/'.$last_date.'/'.$range_report_imutu));
      }
    }
    else if($mode=='data'){
      echo json_encode($this->m_report->laporan_all($data['id'],$data['id2'],$data['id3'],1));
    }
    else{
      if($mode=='tambah'){
        $data['page'] =  $data['page']."_tambah";
