...
/*    public function child_datatable()
    {
        $pegawai = (int) $this->input->post('id_pegawai');
        $dt      = $this->datatable_request();
...
/*    public function child_detail()
    {
        ...
        $d  = $this->m_ol_validasi->detail_pegawai($id);

        if (!$d) {
            echo json_encode(['status'=>false,'html'=>'Data tidak ditemukan']);
            return;
        }

        $html = $this->load->view('ol_validasi/child_detail', $d, true);
	...
	function validasi($mode='view'){
		...
		if($mode=='child_pegawai'){
			...
			echo json_encode($this->m_ol_validasi->logbook_all($data['id']));
		}
		else{
...
{
	$this->load->view("button/header",$data);
	$this->load->view("button/isi");
	$this->load->view("footer");
	$this->load->view("button/jsload");
	$this->load->view("button/jscode");
}
function tampil_ra($data)
{
	$this->load->view("header_top_ra",$data);
	$this->load->view("button/menu");
	$this->load->view("button/isi");
