...
// -----------------------------------------------------------START-----------------------------------------
  ...
  function cek_login_kah()
  {
    ...
    $barpendufe = $this->m_umum->ambil_data('pendaftaran_unit','barcode_pendaftaran_unit',$data['first_date']);
  echo json_encode($this->m_radiologi->pemeriksaan_penunjang_all($barpendufe['id_pendaftaran']));
  }
	else{
    ...
    if($mode=='penunjang'){
      ...
      $kondisi=array('id_pendaftaran'=>$data['first_date']); //id_pendaftaran
      ...
      if($data['jml'] == 0){
        ...
        $d = $this->m_umum->ambil_data('pemeriksaan_ku','id_pendaftaran',$data['first_date']);
    ...
    if($mode=='hapus_penunjang'){
      ...
      $pmr_pen = $this->m_umum->ambil_data('pemeriksaan_penunjang','id_pemeriksaan_penunjang',$data['first_date']);
      $pmr_nama = $this->m_umum->ambil_data('tindakan','id_tindakan',$pmr_pen['id_tindakan']);
  		$data['nama_tindakan'] = set_value('nama_tindakan',$pmr_nama["nama_tindakan"]);
  		$data['id_pemeriksaan_penunjang'] = set_value('id_pemeriksaan_penunjang',$pmr_pen["id_pemeriksaan_penunjang"]);
      $this->load->view("radiologi/isi",$data);
    }
