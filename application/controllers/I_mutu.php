...
class I_mutu extends CI_controller
{
// -----------------------------------------------------------START-----------------------------------------
...
/*		$data['page']="home";
  ...
  function lhu($mode='view')
  {
		...
		$data['id4'] = $this->uri->segment(6, 0);
	//  $data['ambil_data_unit'] = $this->m_im->ambil_data_rujukan_ol_unit($this->session->list_unit);
	  $data['ambil_data_unit'] = $this->m_ol_rancak->ambil_data_rujukan_kab_working($this->session->mas_ins);
    ...
    if($mode=='view'){
		...
		if(empty($data['id2'])){
			...
			if($action == 'BtnProses') {
				...
				redirect(base_url('i_mutu/lhu/view/'.$first_date.'/'.$last_date.'/'.$id4));
			}			
		}
    else if($mode=='data'){
			if(empty($data['id'])){
			...
			echo json_encode($this->m_im->lhu_all($data['id'],$data['id2'],$data['id4']));
		}
    else if($mode=='data2'){
			echo json_encode($this->m_im->lhu_detil_all($data['id']));
		}
/*	  else if($mode=='hapus'){
	  		...
	  		if($jml == 0){
				...
				$cek_file=FCPATH.'assets/berkas/i_mutu/'.$user_pic['link_lhu'];
				if(file_exists($cek_file)){
					unlink(FCPATH."assets/berkas/i_mutu/".$user_pic['link_lhu']);
				}

	  		  if($this->m_umum->hapus_data('sn_lhu','id_lhu',$data['id'])){
	    			$this->session->set_flashdata('sukses', 'Data Sudah Di Hapus');
	    			redirect(base_url('i_mutu/lhu'));
	  		  }else{
...
/*	  		 }else{
	    			...
	    			redirect(base_url('i_mutu/lhu/edit/'.$data['idpeg']));
	  		 }
	  }*/
  	else{
	      ...
	      if($mode=='tambah'){
	        $data['page'] =  $data['page']."_tambah";
					...
					$this->load->view("i_mutu/isi",$data);
	      }
	      if($mode=='simpan_tambah'){
							if($this->input->post('id_standar_mutu')){ 
							  if($this->m_im->simpan_sn_lhu()){
      ...
      if($mode=='upload'){
					...
					$this->tampil($data);
						...
						for($i = 0; $i < $filesCount; $i++){
		  				...
		  				if($this->upload->do_upload('upload_File')){
								...
								$cek_file=FCPATH.'assets/berkas/im/'.$user_pic['link_lhu'];
								if(file_exists($cek_file)){
									unlink(FCPATH."assets/berkas/im/".$user_pic['link_lhu']);
								}
		  					$fileData = $this->upload->data();
		  					$this->session->set_flashdata('sukses', 'Data berhasil Di Upload');
		  					$this->m_im->upload_sn_lhu($fileData['file_name']);
		  					redirect(base_url('i_mutu/lhu'));
		  				}else{
	    ...
	    if($mode=='input'){
	      $data['page'] =  $data['page']."_input";
					...
					$data['pembuat_lhu']  = set_value('pembuat_lhu',$keuangan["pembuat_lhu"]);
	      ...
	      if ($this->form_validation->run() === FALSE){
	        $this->tampil($data);
						...
						$pembuat_lhu = $this->input->post('pembuat_lhu');
						$barcode_lhu = $this->input->post('barcode_lhu');
						$action = $this->input->post('action');	      	
					if($action == 'BtnSave') {
						if($pembuat_lhu == $this->session->id_pegawai){
						  if($this->m_im->edit_sn_lhu()){
								...
								$this->session->set_flashdata('danger', 'Bukan pembuat laporan');
								redirect(base_url('i_mutu/lhu/input/'.$barcode_lhu));							
						}
					}
	      }
      ...
      if($mode=='isi'){
        $data['page'] =  $data['page']."_isi";
				$keuangan = $this->m_umum->ambil_data('sn_lhu','barcode_lhu',$data['id']);
				$data['ambil_limbah']=$this->m_im->ambil_limbah_no_null($keuangan['id_standar_mutu']);
				$data['id_lhu']  = set_value('id_lhu',$keuangan["id_lhu"]);
				$data['barcode_lhu']  = set_value('barcode_lhu',$keuangan["barcode_lhu"]);
					...
					$data['total_lhu_detil']  = set_value('total_lhu_detil',$this->input->post("total_lhu_detil"));
      ...
      if($mode=='rubah'){
        $data['page'] =  $data['page']."_rubah";
				...
				$data['barcode_lhu']  = set_value('barcode_lhu',$keuangan["barcode_lhu"]);
					...
					$data['total_lhu_detil']  = set_value('total_lhu_detil',round($lhud["total_lhu_detil"],3));
...
/*      	$id_lhu = $this->input->post('id_lhu');
      	$id_limbah = $this->input->post('id_limbah');
				...
				$jml = $this->m_umum->jumlah_record_filter('sn_lhu_detil',$kondisi_peg);
				if($jml == 0){ */     	
				  if($this->m_im->edit_sn_lhu_detil()){
						$this->session->set_flashdata('sukses', 'Barang berhasil Di Simpan');
						redirect(base_url('i_mutu/lhu/input/'.$barcode_lhu));
				  }else{
						$this->session->set_flashdata('danger', 'Ada Masalah Penambahan Data');
...
/*				}else{
  ...
  function laporan($mode='view')
  {
		...
		$data['id2'] = $this->uri->segment(5, 0);
		$data['id4'] = $this->uri->segment(6, 0);
		$data['id5'] = $this->uri->segment(7, 0);
		$data['id6'] = $this->uri->segment(8, 0);
    ...
    if($mode=='view'){
			...
			if($action == 'BtnProses') {
				...
				redirect(base_url('i_mutu/laporan/view/'.$first_date.'/'.$last_date.'/'.$id_standar_mutu.'/'.$id5.'/'.$id6));
			}
		}
    else if($mode=='data'){
			if(empty($data['id'])){
			...
			echo json_encode($this->m_im->laporan_all($data['id'],$data['id2'],$data['id4'],$data['id5'],$data['id6']));
		}
    else if($mode=='data2'){  	
			echo json_encode($this->m_im->laporan_tabel_all($data['id']));
		}
    else if($mode=='pie'){
    	$ambil_semua_record_kondisi = $this->m_im->ambil_semua_record_kondisi($data['id2']);
			echo json_encode($this->m_im->grafik_pie($data['id2'],$ambil_semua_record_kondisi));
		}
    else if($mode=='batang_persen'){
    	$ambil_semua_record_kondisi = $this->m_im->ambil_semua_record_kondisi($data['id2']);
			echo json_encode($this->m_im->grafik_batang_persen($data['id2'],$ambil_semua_record_kondisi));
		}
    else if($mode=='batang_range'){
			echo json_encode($this->m_im->grafik_batang_range($data['id2']));
		}
  	else{
      if($mode=='tambah'){
        $data['page'] =  $data['page']."_tambah";
					...
					$data['jenis_laporan']  = set_value('jenis_laporan',$this->input->post("jenis_laporan"));
					$data['satuan_laporan']  = set_value('satuan_laporan',$this->input->post("satuan_laporan"));
					$data['metode_laporan']  = set_value('metode_laporan',$this->input->post("metode_laporan"));
					...
					$data['penyajian_laporan']  = set_value('penyajian_laporan',$this->input->post("penyajian_laporan"));
					$data['kriteria_laporan']  = set_value('kriteria_laporan',$this->input->post("kriteria_laporan"));
					$data['formula_laporan']  = set_value('formula_laporan',$this->input->post("formula_laporan"));
					$data['instrumen_laporan']  = set_value('instrumen_laporan',$this->input->post("instrumen_laporan"));
					$data['sampel_laporan']  = set_value('sampel_laporan',$this->input->post("sampel_laporan"));
        ...
        if ($this->form_validation->run() === FALSE){
					...
					$this->m_im->simpan_sn_laporan();
					$this->session->set_flashdata('sukses', 'Data Sudah Di Tambah');
					redirect(base_url('i_mutu/laporan'));
        }
      }
     if($mode=='edit'){
        $data['page'] =  $data['page']."_edit";
					...
					$data['jenis_laporan']  = set_value('jenis_laporan',$keuangan["jenis_laporan"]);
					$data['satuan_laporan']  = set_value('satuan_laporan',$keuangan["satuan_laporan"]);
					$data['metode_laporan']  = set_value('metode_laporan',$keuangan["metode_laporan"]);
					$data['sampel_laporan']  = set_value('sampel_laporan',$keuangan["sampel_laporan"]);
					$data['tujuan_laporan']  = set_value('tujuan_laporan',$keuangan["tujuan_laporan"]);
					$data['kriteria_laporan']  = set_value('kriteria_laporan',$keuangan["kriteria_laporan"]);
					$data['formula_laporan']  = set_value('formula_laporan',$keuangan["formula_laporan"]);
					$data['penyajian_laporan']  = set_value('penyajian_laporan',$keuangan["penyajian_laporan"]);
					$data['definisi_laporan']  = set_value('definisi_laporan',$keuangan["definisi_laporan"]);
					$data['instrumen_laporan']  = set_value('instrumen_laporan',$keuangan["instrumen_laporan"]);
      ...
      if($mode=='simpan_urutan'){
				...
				if($this->input->post("urutan_laporan_tabel")){  	
				  if($this->m_im->edit_urutan_laporan_tabel()){
						$this->session->set_flashdata('sukses', 'Data berhasil Di Simpan');
						redirect(base_url('i_mutu/laporan/edit/'.$barcode_laporan));
				  }else{
						$this->session->set_flashdata('danger', 'Ada Masalah Penambahan Data');
						redirect(base_url('i_mutu/laporan/edit/'.$barcode_laporan));
				  }
      ...
      if($mode=='regulasi'){
        ...
        $data['title'] = "UPLOAD REGULASI / BERKAS TERKAIT";
				...
				$data['ol_berkas']  = $this->m_im->ol_berkas_not_in($pg['regulasi'],'50');
				$data['ol_berkas_in']  = $this->m_im->ol_berkas_in($pg['regulasi'],'50');
				...
				$data['link_awal'] = base_url('i_mutu/laporan');
				...
				if($action == 'BtnUpload') {
		  		$barcode_laporan = $this->input->post("barcode_laporan");
						...
						for($i = 0; $i < $filesCount; $i++){
		  				...
		  				if($this->upload->do_upload('upload_File')){
		  					$fileData = $this->upload->data();
		  					$last_id = $this->m_im->simpan_berkas_file($fileData['file_name'],'50');
		  					$this->m_im->edit_laporan_regulasi($barcode_laporan,$last_id);
		  					$this->session->set_flashdata('sukses', 'Data berhasil Di Upload');
								redirect(base_url('i_mutu/laporan/regulasi/'.$barcode_laporan));
		  				}else{
				        $this->session->set_flashdata('danger', 'Data Gagal Di Upload');
								redirect(base_url('i_mutu/laporan/regulasi/'.$barcode_laporan));
		  				}
...
/*					}else{
      ...
      if($mode=='berkas'){
        ...
        $data['title'] = "UPLOAD BERKAS";
				$pg = $this->m_umum->ambil_data('sn_laporan','barcode_laporan',$data['id']);
				$data['ol_berkas']  = $this->m_im->ol_berkas_not_in($pg['berkas_laporan'],'50');
				$data['ol_berkas_in']  = $this->m_im->ol_berkas_in($pg['berkas_laporan'],'50');
				$data['barcode_laporan']  = $pg['barcode_laporan'];
				$data['judul_laporan']  = $pg['judul_laporan'];
		  		...
		  		$barcode_laporan = $this->input->post("barcode_laporan");
						...
						for($i = 0; $i < $filesCount; $i++){
		  				...
		  				if($this->upload->do_upload('upload_File')){
		  					$fileData = $this->upload->data();
		  					$last_id = $this->m_im->simpan_berkas_file($fileData['file_name'],'50');
		  					$this->m_im->edit_laporan_berkas_laporan($barcode_laporan,$last_id);
		  					$this->session->set_flashdata('sukses', 'Data berhasil Di Upload');
								redirect(base_url('i_mutu/laporan/berkas/'.$barcode_laporan));
		  				}else{
				        $this->session->set_flashdata('danger', 'Data Gagal Di Upload');
								redirect(base_url('i_mutu/laporan/berkas/'.$barcode_laporan));
		  				}
...
/*					}else{
      ...
      if($mode=='struktur'){
        ...
        $data['title'] = "UPLOAD STRUKTUR ORGANISASI";
				$pg = $this->m_umum->ambil_data('sn_laporan','barcode_laporan',$data['id']);
				$data['ol_berkas']  = $this->m_im->ol_berkas_not_in($pg['struktur_organisasi'],'60');
				$data['ol_berkas_in']  = $this->m_im->ol_berkas_in($pg['struktur_organisasi'],'60');
				$data['barcode_laporan']  = $pg['barcode_laporan'];
				$data['judul_laporan']  = $pg['judul_laporan'];
				...
				if($action == 'BtnUpload') {
		  		...
		  		//			$this->do_resize($fileData['file_name']);
						    ...
						    if ( ! $this->image_lib->resize()){ 
		  					...
		  					$last_id = $this->m_im->simpan_berkas_file($fileData['file_name'],'60');
		  					$this->m_im->edit_laporan_struktur($barcode_laporan,$last_id);
		  					$this->session->set_flashdata('sukses', 'Data berhasil Di Upload');
								redirect(base_url('i_mutu/laporan/struktur/'.$barcode_laporan));
		  				}else{
				        $this->session->set_flashdata('danger', 'Data Gagal Di Upload');
								redirect(base_url('i_mutu/laporan/struktur/'.$barcode_laporan));
		  				}
...
/*					}else{
      ...
      if($mode=='galeri'){
        ...
        $data['title'] = "UPLOAD GAMBAR";
				$pg = $this->m_umum->ambil_data('sn_laporan','barcode_laporan',$data['id']);
				$data['ol_berkas']  = $this->m_im->ol_berkas_not_in($pg['galeri_laporan'],'60');
				$data['ol_berkas_in']  = $this->m_im->ol_berkas_in($pg['galeri_laporan'],'60');
				$data['barcode_laporan']  = $pg['barcode_laporan'];
				$data['judul_laporan']  = $pg['judul_laporan'];
				...
				if($action == 'BtnUpload') {
		  		...
		  		//			$this->do_resize($fileData['file_name']);
						    ...
						    if ( ! $this->image_lib->resize()){ 
		  					...
		  					$last_id = $this->m_im->simpan_berkas_file($fileData['file_name'],'60');
		  					$this->m_im->edit_laporan_gambar($barcode_laporan,$last_id);
		  					$this->session->set_flashdata('sukses', 'Data berhasil Di Upload');
								redirect(base_url('i_mutu/laporan/galeri/'.$barcode_laporan));
		  				}else{
				        $this->session->set_flashdata('danger', 'Data Gagal Di Upload');
								redirect(base_url('i_mutu/laporan/galeri/'.$barcode_laporan));
		  				}
...
/*					}else{
      ...
      if($mode=='print_laporan'){
        ...
        $lptab = $this->m_im->ambil_sn_laporan_detil($data['id2']);
     ...
     if($mode=='tambah_tabel'){
        ...
        if ($this->form_validation->run() === FALSE){
        	...
        	$barcode_laporan = $this->input->post("barcode_laporan");
				  if($this->m_im->tambah_sn_laporan_tabel()){
     ...
     if($mode=='edit_tabel'){
        $data['page'] =  $data['page']."_edit_tabel";
				...
				$data['max_tanggal'] = $this->m_im->max_tanggal_lhu($data['id2']);
				$data['min_tanggal'] = $this->m_im->min_tanggal_lhu($data['id2']);
				$data['only_blnyear_lhu'] = $this->m_im->only_blnyear_lhu($data['id2'],$data['min_tanggal'],$data['max_tanggal']);
				$data['tabel_limbah_detil'] = $this->m_im->tabel_limbah_detil($data['id2']);
				$data['rec_baku_mutu'] = $this->m_im->rec_baku_mutu($data['id2']);
				$data['max_standar'] = round($this->m_im->max_standar_mutu($data['id2'],3))+1;
				$data['min_range'] = round($this->m_im->min_range_mutu($data['id2'],3))-1;
/*				$min_standar = round($this->m_im->min_standar_mutu($data['id2'],3));
				$min_range = round($this->m_im->min_range_mutu($data['id2'],3));
				$min_hasil = round($this->m_im->min_hasil($data['id2'],3));*/
/*				if($min_range == 0){
        ...
        $data['jumlah_bulan'] = $data['jumlah_bulan'] + 1;
				...
				$data['ambil_limbah_grafik'] = $this->m_im->ambil_limbah_grafik_aza($data['id2']);
				$data['grafik_garis_opsi'] = $this->m_im->grafik_garis_opsi($data['id2']);
				$data['ambil_dt_limbah_grafik'] = $this->m_im->ambil_dt_limbah_grafik($data['id2']);		
				$data['grafik_batang_range_jejer'] = $this->m_im->grafik_batang_range_jejer($data['id2']);
				$data['grafik_batang_range'] = $this->m_im->tabel_limbah_detil($data['id2']);		
				$data['grafik_batang_range2'] = $this->m_im->grafik_only_limbah($data['id2']);
				$data['grafik_batang_kelola'] = $this->m_im->grafik_batang_kelola($data['id2']);
/*					
...
*/
        ...
        if ($this->form_validation->run() === FALSE){
        	...
        	$barcode_laporan = $this->input->post("barcode_laporan");
					if($this->m_im->edit_sn_laporan_tabel()){
      ...
      if($mode=='rubah_data'){
        ...
        $data['ambil_data_sn_lhu'] = $this->m_im->ambil_data_sn_lhu($lp['tgl_awal'],$lp['tgl_akhir'],$lp['id_standar_mutu']);
        $data['ambil_limbah'] = $this->m_im->ambil_limbah($lp['id_standar_mutu']);
        $data['tabelkah'] = $this->m_im->ambil_sn_tabel();
        $data['tabel']  = set_value('tabel',$lptab["tabel"]);
        if($lptab["id_lhu"] == 0){
        	$data['semua']  = 0;
        }else{
      ...
      if($mode=='simpan_rubah_data'){
		  	...
		  	$barcode_laporan_tabel = $this->input->post('barcode_laporan_tabel');
				if($this->m_im->edit_input_tabel()){
				  $this->session->set_flashdata('sukses', 'Data Berhasil Disimpan');
					redirect(base_url('i_mutu/laporan/edit_tabel/'.$barcode_laporan.'/'.$barcode_laporan_tabel));
				}else{
					$this->session->set_flashdata('danger', 'Ada Masalah Edit Data');
					redirect(base_url('i_mutu/laporan/edit_tabel/'.$barcode_laporan.'/'.$barcode_laporan_tabel));
				}
      ...
      if($mode=='rubah_limbah'){
        $data['page'] =  $data['page']."_rubah_limbah";
				...
				$data['id_limbah']  = set_value('id_limbah',$lptab["id_limbah"]);
        $data['semuakah'] = array('0'=>'Semua Data','1'=>'Sesuai Check List');
        $data['ambil_data_sn_lhu_detil'] = $this->m_im->ambil_data_sn_lhu_detil($lp['tgl_awal'],$lp['tgl_akhir'],$lp['id_standar_mutu']);
      ...
      if($mode=='simpan_rubah_limbah'){
		  	...
		  	$barcode_laporan = $this->input->post('barcode_laporan');
		  	$barcode_laporan_tabel = $this->input->post('barcode_laporan_tabel');
				if($this->m_im->edit_input_limbah()){
				  $this->session->set_flashdata('sukses', 'Data Berhasil Disimpan');
					redirect(base_url('i_mutu/laporan/edit_tabel/'.$barcode_laporan.'/'.$barcode_laporan_tabel));
				}else{
					$this->session->set_flashdata('danger', 'Ada Masalah Edit Data');
      ...
      if($mode=='rubah_ukur'){
        ...
        $data['semuakah'] = array('0'=>'Semua Data','1'=>'Sesuai Check List');
        $data['ambil_data_sn_lhu_detil'] = $this->m_im->ambil_data_sn_lhu_detil_sumber($lp['tgl_awal'],$lp['tgl_akhir'],$lp['id_standar_mutu']);
      ...
      if($mode=='simpan_rubah_ukur'){
			...
			if($mode=='pdf_intro'){
				$d	= $this->m_im->ambil_sn_laporan_4_print($data['id']);
  ...
  function lihat($mode='view')
  {
    ...
    if($mode=='view'){
			...
			if($action == 'BtnProses') {
				...
				$id5 = $this->input->post("id5");  // tanggal
				$id6 = $this->input->post("id6");  // kategori
				redirect(base_url('i_mutu/lihat/view/'.$first_date.'/'.$last_date.'/'.$id_standar_mutu.'/'.$id5.'/'.$id6));
			}
		}
    else if($mode=='data'){
			if(empty($data['id'])){
			...
			echo json_encode($this->m_im->grafik_pie($data['id2']));
		}
    else if($mode=='garis'){
			echo json_encode($this->m_im->grafik_garis($data['id2']));
		}
    else if($mode=='pie_all'){
			echo json_encode($this->m_im->grafik_pie_all($data['id2']));
		}
    else if($mode=='batang_range'){
			echo json_encode($this->m_im->grafik_batang_range($data['id2']));
		}
      ...
      if($mode=='profil'){
        ...
        $lptab = $this->m_umum->ambil_data('sn_laporan','barcode_laporan',$data['id']);
        $data['ambil_sn_laporan_tabel'] = $this->m_im->ambil_sn_laporan_tabel($data['id']);
      ...
      if($mode=='tabel'){
        ...
        $lptab = $this->m_im->ambil_sn_laporan_detil($data['id2']);
				...
				$data['min_tanggal'] = $this->m_im->min_tanggal_lhu($data['id2']);
        $data['jumlah_bulan'] = $this->m_rancak->hitung_jumlah_bulan($data['min_tanggal'],$data['max_tanggal']);
				$data['max_standar'] = round($this->m_im->max_standar_mutu($data['id2'],3));
				$min_standar = round($this->m_im->min_standar_mutu($data['id2'],3));
				$min_range = round($this->m_im->min_range_mutu($data['id2'],3));
				$min_hasil = round($this->m_im->min_hasil($data['id2'],3));
				if($min_range == 0){
				...
				$data['jumlah_record_tabel_limbah_detil'] = $this->m_im->jumlah_record_tabel_limbah_detil($data['id2']);
				$data['jumlah_record_tps'] = $this->m_im->jumlah_record_tps($data['id2']);
				$data['only_blnyear_lhu'] = $this->m_im->only_blnyear_lhu($data['id2'],$data['min_tanggal'],$data['max_tanggal']);
				$data['tabel_limbah_detil'] = $this->m_im->tabel_limbah_detil($data['id2']);
        $data['ambil_sn_laporan_tabel'] = $this->m_im->ambil_sn_laporan_tabel($data['id']);
        $data['ambil_berkas_lhu'] = $this->m_im->ambil_berkas_lhu($lptab["id_lhu"],$lptab["id_standar_mutu"],$lptab["tgl_awal"],$lptab["tgl_akhir"]);
        $data['count_berkas_lhu'] = $this->m_im->count_berkas_lhu($lptab["id_lhu"],$lptab["id_standar_mutu"],$lptab["tgl_awal"],$lptab["tgl_akhir"]);
				$data['barcode_laporan_tabel']  = set_value('barcode_laporan_tabel',$lptab["barcode_laporan_tabel"]);
				$data['id_laporan_tabel']  = set_value('id_laporan_tabel',$lptab["id_laporan_tabel"]);
				$data['barcode_laporan']  = set_value('barcode_laporan',$lptab["barcode_laporan"]);
				$data['id_laporan']  = set_value('id_laporan',$lptab["id_laporan"]);		
  ...
  function berkas($mode='view')
  {
    ...
    if($mode=='view'){
			...
			echo json_encode($this->m_im->berkas_all());
		}
	  else if($mode=='hapus'){ 
			$this->m_umum->hapus_data('ol_berkas','id_berkas',$data['id']);

				$this->session->set_flashdata('sukses', 'Data berhasil Di Hapus');
				redirect(base_url('i_mutu/berkas'));
	  }
/*    else if($mode=='hapus'){
...
//    $this->image_lib->resize();
  ...
  function laporans($mode='view')
  {
		...
		$data['ambil_data_unit'] = $this->m_ol_rancak->ambil_data_rujukan_kab_working($this->session->mas_ins);
		$id_in =  array(1,3);
		$data['ambil_sn_standar_mutu'] = $this->m_im->ambil_sn_standar_mutu($id_in);
		$data['cmd_jabatan'] = $this->m_rancak->cmd_jabatan();
	  $data['all_kah'] = array('0'=>'Range Tanggal','1'=>'Semua');
	  $data['kategori_kah'] = array('0'=>'Semua','1'=>'Sesuai Kategori');
    if($mode=='view'){
			...
			if($action == 'BtnProses') {
				...
				$id5 = $this->input->post("id5");  // tanggal
				$id6 = $this->input->post("id6");  // kategori
				redirect(base_url('i_mutu/laporan/view/'.$first_date.'/'.$last_date.'/'.$id_instansi.'/'.$id_standar_mutu.'/'.$id5.'/'.$id6));
			}
		}
    else if($mode=='data'){
			if(empty($data['id'])){
			...
			echo json_encode($this->m_im->laporan_all($data['id'],$data['id2'],$data['id3'],$data['id4'],$data['id5'],$data['id6']));
		}
    else if($mode=='data2'){  	
			echo json_encode($this->m_im->laporan_tabel_all($data['id']));
		}
...
*/
      ...
      if($mode=='simpan_rubah_limbah'){
			...
			if($mode=='pdf_intro'){
				$d	= $this->m_im->ambil_sn_laporan_4_print($data['id']);
				$report = $this->load->view('i_mutu/z_intro', $data, TRUE);
			  $filename = $d["nama_standar_mutu"]."-".$d["judul_laporan"]."-print-date-".date('d-m-Y H:i:s').".pdf";
			  $mpdf = new \Mpdf\Mpdf(['mode' => 'utf-8', 'format' => 'Legal-P']);
			  $mpdf->AddPage('P', '', '', '', 2, 5, 5, 5, 10, 3, 3);
			  $mpdf->SetDisplayMode('fullpage');
	...
	/* 		$mpdf->WriteHTML('Your Foreword and Introduction');
	 ...
	 $pdf->SetHTMLHeader('<img src="' . base_url() . 'custom/Hederinvoice.jpg"/>');
	    ...
	    $pdf->WriteHTML($html);
			...
			if($mode=='pdf_table'){
				$d	= $this->m_im->ambil_sn_laporan_4_print($data['id']);
				$report = $this->load->view('i_mutu/z_tabel', $data, TRUE);
			  $filename = $d["nama_standar_mutu"]."-".$d["judul_laporan"]."-table-print-date-".date('d-m-Y H:i:s').".pdf";
			  $mpdf = new \Mpdf\Mpdf(['mode' => 'utf-8', 'format' => 'Legal-P']);
			  $mpdf->AddPage('P', '', '', '', 2, 5, 5, 5, 10, 3, 3);
			  $mpdf->SetDisplayMode('fullpage');
...
{
	$this->load->view("i_mutu/header",$data);
	$this->load->view("i_mutu/isi");
	$this->load->view("footer");
	$this->load->view("i_mutu/jsload");
	$this->load->view("i_mutu/jscode");
}
