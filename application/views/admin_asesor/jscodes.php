        ...
        <div class="p-3 bg-light border">
            <div class="row">
                <div class="col-md-12 border-end">
                    <h6 class="fw-bold text-primary mb-2"><i class="fa fa-users"></i> DAFTAR VALIDATOR</h6>
                    <table class="table table-bordered table-sm bg-white w-100" id="child-validator-${id}">
                ...
                <div class="col-md-12">
                    <h6 class="fw-bold text-success mb-2"><i class="fa fa-pencil-alt"></i> DATA SIGNATURE</h6>
                    <table class="table table-bordered table-sm bg-white w-100" id="child-signature-${id}">
                        <thead class="table-dark">
                            <tr>
                                <th>Tanggal</th>
                                <th>Header / No</th>
...
/*                render: function(data, type, row) {
    ...
    $('#child-signature-' + childId).DataTable({
            destroy: true,
            processing: true,
            serverSide: true,
            paging: false,
            searching: false,
            ajax: {
                url: base_url + 'admin_asesor/pengajuan_kompetensi/child_signature',
                type: 'POST',
                data: { barcode_pengajuan: d.barcode_pengajuan }
            },
            columns: [
                { data: 'tanggal' },
                { 
                    render: function(data, type, row) {
                        return `<b>${row.header}</b><br><small>${row.no}</small>`;
                    }
                },
                { data: 'lampiran' }
            ]
