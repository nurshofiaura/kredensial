...
<?php
  // Tentukan href
  if (!empty($c['target']) && $c['target'] === 'id') {
    $href = '#'.$c['id'];           // contoh: #modal-default
  } elseif (!empty($c['target']) && $c['target'] === 'class') {
    $href = $c['url'];              // tetap url, class akan di-handle JS
  } else {
    $href = $c['url'];              // target kosong → langsung url
  }
?>
<div class="col-12 col-sm-6 col-md-4 col-lg-3 col-xl-2">
  <a href="<?= $href ?>"
     class="text-decoration-none ra-card <?= $c['class'] ?? '' ?>"
     data-jenis="<?= $c['jenis'] ?? '' ?>"
    ...
    <div class="card h-100 text-center dashboard-card">
      <div class="card-body d-flex flex-column align-items-center justify-content-center">
        <img src="<?= $c['image'] ?>" width="60" height="60" class="mb-2">
...
<?php endforeach; ?>
        ...
        <div class="card chat-card">
          ...
          <div class="card-body">
            <div class="d-flex align-items-center mb-2">
              ...
              <div class="btn-group">
                <button class="btn btn-sm btn-secondary" id="btnToday">Hari Ini</button>
                <button class="btn btn-sm btn-secondary" id="btnPrev">‹</button>
                <button class="btn btn-sm btn-secondary" id="btnNext">›</button>
              </div>

              <h4 id="calTitle" class="m-0 flex-grow-1 text-center"></h4>

              <div class="btn-group">
                <button class="btn btn-sm btn-outline-primary" data-view="dayGridMonth">Month</button>
                <button class="btn btn-sm btn-outline-primary" data-view="timeGridWeek">Week</button>
                <button class="btn btn-sm btn-outline-primary" data-view="timeGridDay">Day</button>
                <button class="btn btn-sm btn-outline-primary" data-view="listMonth">List</button>
              </div>
    ...
    <div class="row mb-4">
      ...
      <div class="col-md-6">
        <div class="card h-100">
          <div class="card-header">
            <h5>Grafik Line Pelatihan</h5>
          </div>
          <div class="card-body">
            <div id="pie2" class="chart-lg"></div>
          </div>
        </div>
      </div>
    </div>
  ...
  <div class="modal-dialog modal-dialog-centered modal-lg">
    <div class="modal-content">
      <div class="modal-header <?= $md ?> ">
        <h5 class="modal-title text-white" id="raModalTitle">Primary Modal</h5>
...
<div class="modal fade" id="eventModal">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-header <?= $md ?>">
        <h5 class="modal-title"><div style="color: white;"> Event</div></h5>
        <button class="btn-close" data-bs-dismiss="modal"></button>
      </div>

      <div class="modal-body">
        <input id="eventTitle" class="form-control mb-2" placeholder="Judul">
        <textarea id="eventDesc" class="form-control mb-2" placeholder="Deskripsi"></textarea>
        <input type="color" id="eventColor" class="form-control form-control-color">
      </div>

      <div class="modal-footer">
        <button id="btnSaveEvent" class="btn btn-primary">Simpan</button>
        <button id="btnDeleteEvent" class="btn btn-danger">Hapus</button>
        <button type="button" class="btn btn-light-secondary" data-bs-dismiss="modal">Close</button>
      </div>
...
iframe {
  ...
  border: none;
  overflow: hidden; /* cegah scrollbar keluar */
  display: block;
}
...
<?php if (!empty($billing)): ?>
  ...
  <?php foreach($billing as $rowbilling): ?>
    <?php
      $dateb = date("Y-m-d", strtotime("+3 month"));
      $bg = ($rowbilling['expired_mitra'] > $dateb) ? 'bg-green' : 'bg-red';
    ?>
    <div class="row g-3 align-items-start <?= $bg ?> p-3 rounded mb-3">

      <!-- KIRI : GAMBAR / STRUK (66%) -->
      <div class="col-md-8">
        <div style="width: 100%; max-width: 700px; height: 400px; overflow: hidden; border-radius: 8px; border: 1px solid #ccc;">
          <iframe
            src="<?= base_url('assets/berkas/struk/'.$rowbilling['struk_working_mitra']) ?>"
            loading="lazy"
            style="border:0">
          </iframe>
        </div>
      ...
      <div class="col-md-4">
        <ul class="mt-3 mb-0 ps-3">
          <li><strong>Komite / Bidang:</strong> <?= htmlspecialchars($rowbilling['nama_struktur_jabatan']) ?></li>
          <li><strong>Kontak:</strong> <?= htmlspecialchars($rowbilling['nama_pegawai']) ?></li>
          <li><strong>Expired:</strong> <?= $this->m_rancak->fullBulan(date('d-m-Y', strtotime($rowbilling['expired_mitra']))) ?></li>
          <li class="text-muted">
            Apabila data billing expired, hak akses admin akan hilang
          </li>
        </ul>
      </div>
...
.cover-upload {
    ...
    right: 15px;
    top: 15px;
    z-index: 2; /* biar tombol upload tetap bisa diklik */
}
...
<div class="profile-image" id="coverPreview"
     style="background-image:url('<?= base_url().'assets/images/kredensialban.png' ?>');
            background-size:cover;
            background-position:top center;
            background-repeat:no-repeat;">

    <!-- cover preview clickable 
    <a href="<= base_url().'assets/images/kredensialban.png' ?>"
       class="glightbox cover-lightbox"
       data-glightbox="type: image"
       id="coverLightbox"></a>-->

  <!--  <div class="cover-upload">
        <input type="file" id="coverUpload" accept=".png, .jpg, .jpeg">
        <label for="coverUpload" class="cover-btn">
...
<?php echo form_open_multipart('member/profil',' class="app-form" ');
...
<?php
inputra('nama_pegawai', $nama_pegawai, [
...
<?php
inputra('nip', $nip, [
    'placeholder' => 'Ketikkan NIP',
    'required' => 'required',
    'onkeypress' => 'return event.charCode >= 48 && event.charCode <= 57'
]); 
?><small id="nip_status" class="form-text"></small>
                                      </div>
                                  </div>
                                  <div class="col-4">
                                      <div class="mb-3">
...
?><small id="nik_status" class="form-text"></small>
                                      </div>
                                  </div>
                                  <div class="col-4">
                                      <div class="mb-3">
...
?>
                                  ...
                                  <div class="col-6">
                                      <div class="mb-3">
                                          <label class="form-label">No Profesi</label>
<?php
inputra('no_profesi', $no_profesi, [
    'placeholder' => 'No Profesi (PARI / NIRA DLL)',
    'required' => 'required',
    'onkeypress' => 'return event.charCode >= 48 && event.charCode <= 57'
]); 
?>
...
<?php
inputra('username', $username, [
    'placeholder' => 'USERNAME WAJIB',
    'maxlength' => '60',
    'required' => 'required'
]);
?><small id="username_status" class="form-text"></small>
                                      </div>
                                  </div>
                                  <div class="col-md-6">
                                      <div class="mb-3">
        ...
        <main>
          <div class="container-fluid">
            <div class="row">
              <div class="col-md-12">
                <div class="card">
            <form method="post" action="<?= base_url('ol_logbook/logbook/save_logbook') ?>">
                  <div class="card-header" style="vertical-align:middle;">
                                  ...
                                  <div class="col-4">
                                      <div class="mb-3">
                                          <label class="form-label">Tanggal Lahir</label>
<?php
inputra('tgl_logbook', $tgl_logbook, [
    'class' => 'form-control basic-date form-control',
...
?>
                ...
                <?php foreach($rows as $r): ?>
                    <div class="row mb-3 border p-2 rounded">
                        ...
                        <div class="col-md-6">
                            <b>Kode Unit : <?= $r['kode_unit'] ?></b><br>
                            Nama Kompetensi : <?= $r['nama_kompetensi'] ?><br>
                            <span class="text-success">Nama Kewenangan : <?= $r['nama_kewenangan'] ?></span>
                        </div>

                        <div class="col-md-3">
                            <label>Sifat Kewenangan</label>
<?php
 selectra(
    'id_sifat_kewenangan[]',
    $sifat,
    ...
    [
        'class' => 'form-control select2 select-example',
        'id' => 'id_sifat_'.$r['coun_kewenangan']
    ]
);
?>
                        </div>
                        ...
                        <div class="col-md-3">
                            <label>Jumlah</label>
<?php
inputra('jml_logbook[]', '1', [
    'placeholder' => 'Masukkan Jumlah Logbook',
...
?>
                        ...
                        <input type="hidden" name="id_kewenangan[]" value="<?= $r['coun_kewenangan'] ?>">
...
?>
                  ...
                  <div class="card-footer">
<form id="formProsesLogbook" method="post" action="<?= base_url('ol_logbook/logbook/input_logbook') ?>">
    <button type="submit" id="btnProses" class="btn btn-md btn-primary mb-2">
        Input Data Logbook
    </button>
</form>    
                  </div>
...
$md = $arraymodal[$modalarray];
?>
<div class="modal fade" id="modalAreass" aria-hidden="true" tabindex="-1">
<!-- GANTI UKURAN DI SINI -->
<!-- 
modal-sm | modal-lg | modal-xl | modal-fullscreen | modal-dialog-scrollable 
modal-dialog modal-dialog-centered modal-xl modal-dialog-scrollable
-->
  <div class="modal-dialog modal-dialog-centered modal-lg">
    <div class="modal-content">
      <div class="modal-header <?= $md ?>">
        <h5 class="modal-title text-white">Primary Modal</h5>
...
<div id="action-buttons" class="mb-2 d-flex gap-2 flex-wrap" style="margin-left: 12px;">

  <?= ra_btn_url("Tambah Logbook", base_url("ol_logbook/logbook/tambah"), [
      "icon" => "fa fa-plus",
      "data-size" => "btn-sm"
  ]) ?>
  ...
  <!-- ra_btn("Edit Logbook", [
      "icon" => "fa fa-edit",
      "data-size" => "btn-sm",
      "data-modal-url" => base_url("ol_logbook/logbook/edit/"),
...
<?= ra_btn("Edit Logbook", [
    ...
    "data-size" => "btn-sm",
    "data-modal-url" => base_url("ol_logbook/logbook/edit"),
    "data-modal-title" => "Edit Logbook",
    "data-modal-method" => "GET",
    "data-require-select" => "1"
]) ?>

  <?= ra_btn("Tambah Pasien", [
      "icon" => "fa fa-user-plus",
      "data-size" => "btn-sm",
      "data-modal-url" => base_url("ol_logbook/logbook/pasien_tambah"),
      "data-modal-title" => "Tambah Pasien",
      "data-modal-method" => "GET",
      "data-require-select" => "1"
  ]) ?>

<?= ra_btn_url("Print", "javascript:void(0)", [
    "icon" => "fa fa-print",
    "data-size" => "btn-sm",
    "target" => "_blank",
    "id" => "btnPrint"
...
// Asumsikan $id_kewenangan_lama adalah ID dari database
// Dan $nama_kewenangan_lama adalah teks labelnya
$label_tampil = $id_kewenangan ? ($nama_kewenangan . " [ " . $kode_unit . " ] " . $nama_kompetensi) : '';

selectra_ajax(
    'id_kewenangan', 
    site_url('ol_logbook/get_data_ajax'), 
    $label_tampil, 
    $id_kewenangan, 
    'Cari Kewenangan...'
);
...
$(document).ready(function(){
  ...
  $('#form-logbook-modal').on('submit', function(e){
      ...
      $.ajax({
          url: "<?= base_url('ol_logbook/logbook/rubah_logbook') ?>",
          type: "POST",
          data: formData,
          dataType: "json",
          ...
          complete: function(){
              $btn.prop('disabled', false).text('Simpan Logbook');
          }
      });
  });
});
...
<?php
inputra('nama_pasien', $nama_pasien, [
    'placeholder' => 'Ketikkan Nama',
    'maxlength' => '60',
    'autofocus' => 'autofocus',
    'required' => 'required'
...
<?php
 selectra(
    ...
    FALSE,
    [
        'class' => 'form-control select-example select2-sm'
    ]
);
?>
                                      </div>
...
function initPasienAutocomplete(selector) {
    $(selector).autocomplete({
        minChars: 3,
        type: 'GET',
        serviceUrl: '<?= base_url("ol_logbook/logbook/pasien_cari_data") ?>',
        paramName: 'query', // PENTING
        dataType: 'json',

        onSelect: function (suggestion) {

            $('#nama_pasien').val(suggestion.nama_pasien);
            $('#rm').val(suggestion.rm);
            $('#tgl_lahir').val(suggestion.tgl_lahir);
            $('#jk').val(suggestion.jk).trigger('change');
...
initPasienAutocomplete('#nama_pasien');
initPasienAutocomplete('#rm');
  ...
  $('#form-logbook-modal').on('submit', function(e){
      ...
      $.ajax({
          url: "<?= base_url('ol_logbook/logbook/simpan_pasien') ?>",
          type: "POST",
          data: formData,
          dataType: "json",
          success: function(res){
...
initPasienAutocomplete('#rm');
  ...
  $('#form-logbook-modal').on('submit', function(e){
      ...
      $.ajax({
          url: "<?= base_url('ol_logbook/logbook/update_pasien') ?>",
          type: "POST",
          data: formData,
          dataType: "json",
          success: function(res){
...
elseif ($page=="modal_hapus_pasien")
{
?>
<form id="form-logbook-modal">
            <div class="row">
    ...
    <input type="hidden" name="child_table_id" value="<?= $child_table_id ?>">
                  <div class="card-body">
                      <div class="row">
                        <div class="col-12">
                            <div class="mb-3">
                                <label class="form-label">YAKIN AKAN MENGHAPUS PASIEN : <br>Nama Pasien : <?= $nama_pasien ?></label>
                            </div>
                        </div>
                      </div>
                  </div>
...
$(document).ready(function(){
  ...
  $('#form-logbook-modal').on('submit', function(e){
      ...
      $.ajax({
          url: "<?= base_url('ol_logbook/logbook/delete_pasien') ?>",
          type: "POST",
          data: formData,
          dataType: "json",
          success: function(res){
...
<!--   ra_btn_url("Tambah Logbook", base_url("ol_logbook/logbook/tambah"), [
  ...
  <?= ra_btn("Tambah Laporan", [
      "icon" => "fa fa-plus",
      "data-size" => "btn-sm",
      "data-modal-url" => base_url("member/report/tambah"),
      "data-modal-title" => "Tambah Laporan"
  ])
  ?>
...
<?= ra_btn("Edit Laporan", [
    "icon" => "fa fa-edit",
    "data-size" => "btn-sm",
    "data-modal-url" => base_url("member/report/edit"),
    "data-modal-title" => "Edit Laporan",
    "data-modal-method" => "GET",
    "data-require-select" => "1"
]) ?>

  <?= ra_btn("Tambah Halaman", [
      "icon" => "fa fa-user-plus",
      "data-size" => "btn-sm",
      "data-modal-url" => base_url("member/report/add_sheet"),
      "data-modal-title" => "Tambah Halaman",
      "data-modal-method" => "GET",
      "data-require-select" => "1"
  ]) ?>
...
<?php
inputra('tgl_laporan', $tgl_laporan, [
    'class' => 'form-control basic-date form-control',
    'placeholder' => 'DD-MM-YYYY'
  ]);
?>
...
?>
                      ...
                      <div class="col-6">
                          <div class="mb-3">
                              <label class="form-label">Header Laporan</label>
<?php
inputra('header_laporan', $header_laporan, [
    'placeholder' => 'Header',
    'maxlength' => '255',
    'autofocus' => 'autofocus',
    'required' => 'required'
...
?>
                      ...
                      <div class="col-6">
                          <div class="mb-3">
                              <label class="form-label">Sub Header Laporan 1</label>
<?php
inputra('sub_header_laporan', $sub_header_laporan, [
...
?>
                      ...
                      <div class="col-6">
                          <div class="mb-3">
                              <label class="form-label">Sub Header Laporan 2</label>
<?php
inputra('sub_sub_header_laporan', $sub_sub_header_laporan, [
...
<?php
inputra('judul_laporan', $judul_laporan, [
    'maxlength' => '255',
    'autofocus' => 'autofocus',
    'required' => 'required'
]); 
...
<?php
inputra('tujuan_laporan', $tujuan_laporan, [
    'maxlength' => '255',
    'autofocus' => 'autofocus',
    'required' => 'required'
]); 
...
<?php
inputra('periode_laporan', $periode_laporan, [
    'maxlength' => '255',
    'autofocus' => 'autofocus',
    'required' => 'required'
]); 
...
<?php
inputra('sumber_laporan', $sumber_laporan, [
    'maxlength' => '255',
    'autofocus' => 'autofocus',
    'required' => 'required'
]); 
...
$(document).ready(function(){
  ...
  $('#form-logbook-modal').on('submit', function(e){
      ...
      $.ajax({
          url: "<?= base_url('member/report/simpan') ?>",
          type: "POST",
          data: formData,
          dataType: "json",
          success: function(res){
...
<?php
inputra('judul_laporan_detil', $judul_laporan_detil, [
...
<?php
inputra('urutan_laporan_detil', $urutan_laporan_detil, [
    'maxlength' => '3',
    'autofocus' => 'autofocus',
    'required' => 'required',
    'onkeypress' => 'return event.charCode >= 48 && event.charCode <= 57'
...
<?php
inputra('min_laporan_detil', $min_laporan_detil, [
    'required' => 'required',
...
?>
                      ...
                      <div class="col-3">
                          <div class="mb-3">
                              <label class="form-label">Maximal</label>
<?php
inputra('max_laporan_detil', $max_laporan_detil, [
    'required' => 'required',
    'onkeypress' => 'return event.keyCode > 47 && event.keyCode < 58'
]); 
?>
...
$(document).ready(function(){
  ...
  $('#form-logbook-modal').on('submit', function(e){
      ...
      $.ajax({
          url: "<?= base_url('member/report/save_sheet') ?>",
          type: "POST",
          data: formData,
          dataType: "json",
          success: function(res){
          ...
          complete: function(){
              $btn.prop('disabled', false).text('Simpan Halaman');
          }
      });
  });
});
    ...
    <input type="hidden" name="child_table_id" value="<?= $child_table_id ?>">
                  <div class="card-body">
                      <div class="row">
                        <div class="col-12">
                            <div class="mb-3">
                                <label class="form-label">YAKIN AKAN MENGHAPUS DATA : <br>JUDUL : <?= $judul_laporan_detil ?></label>
                            </div>
                        </div>
                      </div>
                  </div>
...
$(document).ready(function(){
  ...
  $('#form-logbook-modal').on('submit', function(e){
      ...
      $.ajax({
          url: "<?= base_url('member/report/remove_sheet') ?>",
          type: "POST",
          data: formData,
          dataType: "json",
          success: function(res){
          ...
          complete: function(){
              $btn.prop('disabled', false).text('Hapus halaman');
          }
      });
  });
});
    ...
    <section class="content">
      <div class="box box-<?php echo $arraybox[array_rand($arraybox)]; ?> box-solid">
        ...
        <div class="box-body">
			<div class="col-md-6">
<?php 
//echo form_open_multipart('ol_logbook/logbook/view/'.$first_date.'/'.$last_date.'/'.$id_ruangan.'/'.$pxe,' id="signupform" ');  
?>
<form method="post" action="<?= base_url('ol_logbook/logbook/filter') ?>">
			  <div class="box box-<?php echo $arraybox[array_rand($arraybox)]; ?> box-solid">
				<div class="box-header with-border">
...
.select2-container {
  ...
  <div class="content-wrapper">
    ...
    <section class="content">
    <form method="post" action="<?= base_url('ol_logbook/logbook/simpan_logbook') ?>">
	<?php //echo form_open_multipart('ol_logbook/logbook/isi/'.$id_gabungan,' id="signupform" ');
	input_text("id_pegawai",$this->session->id_pegawai,"","","hidden");
	input_text("counter",$count,"","","hidden");
  input_text("id_gabungan",$id_gabungan,"","","hidden");
	?>
      <div class="box box-<?php echo $arraybox[array_rand($arraybox)]; ?> box-solid">
        <div class="box-header with-border">
          <?= $title ?>
