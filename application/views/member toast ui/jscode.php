...
function initUI(){
  ...
  /* ===== FLATPICKR ===== */
  document.querySelectorAll('.basic-date').forEach(el=>{
    if(el._flatpickr){
    ...
    flatpickr(el,{
      ...
      onOpen:function(_,__,fp){
        fp.calendarContainer.addEventListener('mousedown',function(e){
          e.stopPropagation(); // 🔥 PALING PENTING
        });
      }
    });
  });
...
/*function loadJadwal(bulan,tahun){
  ...
  $.ajax({
    url: base_url+'member/jadwal_ajax/'+bulan+'/'+tahun,
    type:'GET',
    success:function(html){

      $('#jadwal-wrapper').html(html);

      // WAJIB: pastikan DOM sudah masuk
...
function loadJadwal(bulan, tahun){
  ...
  $.ajax({
    ...
    success:function(html){
      ...
      initDataTableFixed(); // ⬅️ INI YANG BENER
    }
  });
}
...
function loadJadwalDashboard(bulan,tahun){
  $('#jadwal-wrapper2').html(
    '<div class="text-center p-5"><div class="spinner-border"></div></div>'
  );

  $.get(
    base_url+'member/jadwal_dashboard_ajax/'+bulan+'/'+tahun,
    function(html){
...
function initCalendar(){
  ...
  const calendar = new FullCalendar.Calendar(el,{
    ...
    events: base_url+'member/load',

    dateClick:function(info){
      $('#formAdd')[0].reset();
      $('#modalAdd').modal('show');
      $('[name=start]').val(moment(info.dateStr).format('DD-MM-YYYY'));
      $('[name=end]').val(moment(info.dateStr).format('DD-MM-YYYY'));
    },

    eventClick:function(info){
      $.get(
        base_url+'member/get/'+info.event.id,
        function(res){
          const d = JSON.parse(res);
          $('#id').val(d.id);
          $('#title').val(d.title);
          $('#description').val(d.description);
          $('#start').val(moment(d.start_date).format('DD-MM-YYYY'));
          $('#end').val(moment(d.end_date).format('DD-MM-YYYY'));
          $('#color').val(d.color);
...
/* =========================================================
   CARD / SHORTCUT HANDLER (NO GLOBAL CLICK)
========================================================= */
$(document).on('click','.ra-card',function(e){

  // BIAR GA GANGGU UI
  if($(e.target).closest(
    '.flatpickr-calendar,.select2-container,.dropdown-menu'
  ).length) return;
  ...
  if(jenis === 'modal'){
    const modal = new bootstrap.Modal(document.getElementById(id || 'raModal'));
...
$(function(){
  ...
  initCalendar();   // 🔥 INI WAJIB DI AKHIR
});

function initDataTableFixed(){

  const table = $('#example1');

  if(!table.length){
    console.warn('[DT] table #example1 tidak ditemukan');
    return;
  }
        ...
        var options = {
            ...
            legend: {
                ...
                formatter: function(label, opts) {
                    ...
                    var perc = ((val / total) * 100).toFixed(1); // 1 desimal
                    return label + " : " + opts.w.globals.series[opts.seriesIndex] + ": {" +perc + "% }";
                }
            },
            dataLabels: { enabled: true }, // matikan label di slice
            tooltip: { enabled: false }      // matikan tooltip
        };

        if(chartPie) chartPie.destroy();
        chartPie = new ApexCharts(document.querySelector("#pie1"), options);
        chartPie.render();
    }

    // load awal
    function loadPie(tahun){
        $.ajax({
            url: '<?php echo base_url("member/pie_logbook_ajax"); ?>/' + tahun,
            method: 'GET',
            dataType: 'json',
            success: function(res){
                renderPie(res.labels, res.data, res.colors);
            }
        });
    }

    // inisialisasi chart untuk tahun default
    loadPie($('#tahun_pie').val());
...
*/
function initApex(){
  if(!document.querySelector('#line1')){
    console.warn('[Apex] element #line1 tidak ditemukan');
    return;
  }
  ...
  try {
        ...
        // Parse PHP JSON
        var series_data = <?php echo $series_json; ?>;
        var categories  = <?php echo $categories_json; ?>;

        // ambil index tahun terakhir
        var lastIndex = categories.length - 1;

        // hitung total nilai di tahun terakhir
        var totalLast = 0;
        series_data.forEach(function(s){
            totalLast += Number(s.data[lastIndex] || 0);
        });
        ...
        var chartOptions = {
            ...
            chart: {
                type: 'line',
                height: 550,   // ⬅️ tambah tinggi
                toolbar: { show: true }
            },
            stroke: { curve: 'smooth' },
            xaxis: { categories: categories },
            dataLabels: { enabled: true }, // ❌ matikan
            markers: { size: 4 },
            tooltip: { enabled: false },    // ❌ matikan tooltip
            legend: {
                position: 'bottom',
                ...
                formatter: function(seriesName, opts) {
                    ...
                    var data = opts.w.config.series[seriesIndex].data;
        ...
        var chart = new ApexCharts(document.querySelector("#line1"), chartOptions);
        chart.render();
...
document.addEventListener('DOMContentLoaded', function(){
  ...
  function updateTitle(){
    document.getElementById('calTitle').innerText =
      calendar.getDate().toDate().toLocaleDateString('id-ID',{
        month:'long',
        year:'numeric'
      });
  }
  ...
  calendar.on('selectDateTime', function(e){
    ...
    document.getElementById('evtStart').value = e.start.toISOString();
    document.getElementById('evtEnd').value   = e.end.toISOString();
    document.getElementById('evtTitle').value = '';
    document.getElementById('evtDesc').value  = '';
    document.getElementById('evtColor').value = '#3788d8';
  ...
  document.getElementById('btnSaveEvent').onclick = function(){
    ...
    const schedule = {
      ...
      calendarId: '1',
      title: title,
      body: document.getElementById('evtDesc').value,
      category: 'time',
      start: document.getElementById('evtStart').value,
      end: document.getElementById('evtEnd').value,
      bgColor: document.getElementById('evtColor').value,
      borderColor: document.getElementById('evtColor').value
    };

    calendar.createSchedules([schedule]);

    fetch('<?= site_url("member/add") ?>', {
      method: 'POST',
