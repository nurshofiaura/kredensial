...
?>
  ...
  function format ( d ) {        // `d` is the original data object for the row
    return '<table class="table table-striped text-muted style="padding-left:50px; ">'+
      '<tr> <td>Pembuat:</td><td>'+d.nama_pegawai+'</td> <td></td><td>Alamat:</td><td>'+d.alamat_event+'</td> </tr>'+
      '</table>';
  }
    $(document).ready(function() {
        var table = $('#dttb').DataTable( {
            ...
            "ajax": {
                "url"  : "<?php echo base_url();?>admin_even/<?php echo $page;?>/data",
                "type" : "POST"
            },
            "columns": [
  {
            ...
            "buttons": [
                {
                    ...
                    action: function ( e, dt, node, config ) {
                        $("#modal-default").modal();
                          $('.modal-body').load('<?php echo base_url('admin_even/'.$page.'/tambah'); ?>',function(){
                ...
                },
                {
                  ...
                  className: "btnteal",
                    action: function ( e, dt, node, config ) {
                        ...
                        $("#modal-default").modal();
                          $('.modal-body').load('<?php echo base_url('admin_even/'.$page.'/edit/'); ?>'+data,function(){
                            $('#modal-default').modal({show:true});
                          });
                ...
                {
                    text: "<i class='fa fa-plus-square'></i> Tambah Acara",
                    extend: "selected",
                    className: "btnpurple",
                    action: function ( e, dt, node, config ) {
                        data = dt.rows( { selected: true } ).data()[0]['id_even'];                       
                        // alert(JSON.stringify(data));
                        location.href = '<?php echo base_url('admin_even/'.$page.'/even_detil/'); ?>'+data;
                    }
                },
                {
                    text: "<i class='fa fa-user-plus'></i> Tambah Peserta",
                    extend: "selected",
                    className: "btnorange",
                    action: function ( e, dt, node, config ) {
                        data = dt.rows( { selected: true } ).data()[0]['id_even'];                       
                        // alert(JSON.stringify(data));
                        location.href = '<?php echo base_url('admin_even/'.$page.'/even_peserta/'); ?>'+data;
                    }
                },
                {
...
?>
  ...
  function format ( d ) {        // `d` is the original data object for the row
    ...
    $(document).ready(function() {
        var table = $('#dttb').DataTable( {
            ...
            "ajax": {
                "url"  : "<?php echo base_url();?>admin_even/<?php echo $page;?>/data/<?php echo $this->session->even_acara;?>",
                "type" : "POST"
            },
            "columns": [
  {
  ...
  { "data": "speaker_even_detil","searchable":false, "orderable":false }                
            ...
            "buttons": [
                ...
                {
                    text: "<i class='fa fa-file-text-o'></i> Hasil dan Kesimpulan",
                    extend: "selected",
                    className: "btnpurple",
                    action: function ( e, dt, node, config ) {
                        data = dt.rows( { selected: true } ).data()[0]['id_even_detil'];                       
                        // alert(JSON.stringify(data));
                        location.href = '<?php echo base_url('admin_even/'.$page.'/acara_detil/'); ?>'+data;
                    }
                },
                 {
                     ...
                     swal({
                       title: "HAPUS DATA ?",
                       text: "Yakin Data Akan Di Hapus ?",     //[Modif Disini]
                       icon: "warning",
                       buttons: true,
                       dangerMode: true,
                     })
                     .then((willDelete) => {
                       if (willDelete) {
                         location.href = '<?php echo base_url('admin_even/'.$page.'/hapus/'); ?>'+data; //[Modif Disini]
                       }
                     });
                    }
                 },
...
?>
  ...
  function format ( d ) {        // `d` is the original data object for the row
    return '<table class="table table-striped text-muted style="padding-left:50px; ">'+
      '<tr> <td>Tempat Lahir:</td><td>'+d.tmp_lahir+'</td> <td></td><td>Tanggal Lahir:</td><td>'+d.tgl_lahir+'</td> </tr>'+
      '<tr> <td>Status Kawin:</td><td>'+d.nama_status_kawin+'</td> <td></td><td>Status Pegawai:</td><td>'+d.nama_status_pegawai+'</td> </tr>'+
      '<tr> <td>Gender:</td><td>'+d.jk+'</td> <td></td><td>NIP:</td><td>'+d.nip+'</td> </tr>'+
      '<tr> <td>Pendidikan:</td><td>'+d.nama_pendidikan+'</td> <td></td><td>Agama:</td><td>'+d.nama_agama+'</td> </tr>'+
  ...
  { "data": "email","searchable":false, "orderable":false }                
            ...
            "buttons": [
                {
                    text: "<i class='fa fa-user-plus'></i> Seting Peserta",
                    className: "btnmaroon",
                    init: function(api, node, config) {
                        $(node).removeClass('btn-default')
                    },
