<?php 
	...
	$member = $this->m_umum->ambil_data('pegawai','id_pegawai',$bcp_id_pegawai);
	$fcolor = $adata['fcolor'];
	$color = $adata['color'];
			  ...
			  <tbody>		
				...
				<tr>
					...
					foreach (range(1, $tglakhir) as $numbers) {
						...
						$jml = $this->m_member->jumlah_record_logbook($bcp_id_pegawai,$tglenya,$row['id_kewenangan']);
						if($jml == 0){		
					?>
					<td style="font-size: 0.8em;font-weight:bold;border: 1px solid black;vertical-align:middle;text-align:center">0</td>
					<?php
						}else{
							$q = $this->m_member->total_record_logbook($bcp_id_pegawai,$tglenya,$row['id_kewenangan']);
							foreach($q as $row2){		
					?>
