...
<table class="table">
  ...
  <tbody>		
		<?php
			$no = 0;
			$print_logbook_bulanane = $this->m_member->print_logbook_perbulanane($bcp_id_pegawai,$bt,$id_ruangan,$pxe);
	...
	<tr>
		...
		foreach (range(1, $tglakhir) as $numbers) {
			...
			$jml = $this->m_member->jumlah_record_logbook_kompetensi($bcp_id_pegawai,$tglenya,$row['id_kompetensi'],$id_ruangan,$pxe);
			if($jml == 0){		
		?>
		<td style="font-size: 0.8em;font-weight:bold;border: 1px solid black;vertical-align:middle;text-align:center;">0</td>
		<?php
			}else{
				$q = $this->m_member->total_record_logbook_kompetensi($bcp_id_pegawai,$tglenya,$row['id_kompetensi'],$id_ruangan,$pxe);
				foreach($q as $row2){		
		?>
...
$total = $this->m_member->ambil_bulan_total_logbook_perbulanane($select_total,$kondisi_total,$first_date,$last_date,'nkr_kewenangan.id_kompetensi');
...
<tr>
	...
	<td style="font-size: 0.8em;font-weight:bold;border: 1px solid black;vertical-align:middle;text-align:left"><?= $rowtotal['nama_kompetensi'] ?></td>
