...
<div class="content-report">
	<table class="table table-border">
		<thead>
			<tr class="bg-dark">
			  <th class="left py-1 px-1">Expired</th>
			  <th class="left py-1 px-1">Nama</th>
			  <th class="left py-1 px-1">Nama Berkas</th>
			  <th class="left py-1 px-1">No STR/SIK/SIP</th>				
			</tr>
		</thead>
		<tbody>
			<? 
			foreach ($cmd_ruangan as $rowcmd_ruangan) { 
				$jml=$this->m_berkas->jumlah_berkas_ruangan($id,$rowcmd_ruangan['id_ruangan']);	
				if($jml > 0){
			?>
			<tr class="bg-dark">		
			...
			<?php
				$ambil_data_surat_ijin = $this->m_berkas->ambil_data_surat_ijin($id,$rowcmd_ruangan['id_ruangan'],'0');
