...
//		$ids = explode(',', $unit);
	...
	//--------- Mulai Query UTAMA ---------------------------
	  ...
	  $this->db->from('pemeriksaan_penunjang pp');
		...
		$this->db->where('pp.id_pendaftaran', $id['id_pendaftaran']);
	...
	function simpan_tambah_p_billing($last_id,$harga_tindakan,$id,$jml){
		$data_pendaftaran = array(
			'id_tindakan_tarif' => $id,
			'jml_billing' => $jml,
			'id_pemeriksaan' => $last_id,
	...
	{
		$this->db->select('COUNT(*) as num');
		$this->db->join('pemeriksaan pmr', 'pmr.id_pemeriksaan=b.id_pemeriksaan','left');
		$this->db->join('tindakan_tarif tt', 'tt.id_tindakan_tarif=b.id_tindakan_tarif','left');
		$this->db->join('tindakan t', 't.id_tindakan=tt.id_tindakan','left');
	...
	//--------- Mulai Query UTAMA ---------------------------
		...
		$this->db->join('pemeriksaan pmr', 'pmr.id_pemeriksaan=b.id_pemeriksaan','left');
		$this->db->join('pendaftaran_unit pu', 'pu.id_pendaftaran_unit=pmr.id_pendaftaran_unit','left');
		$this->db->join('kol_kelas kk', 'kk.id_kelas=pmr.id_kelas','left');
		$this->db->join('tindakan_tarif tt', 'tt.id_tindakan_tarif=b.id_tindakan_tarif','left');
		$this->db->join('tindakan t', 't.id_tindakan=tt.id_tindakan','left');
