<?php
class M_ol_validasi extends CI_model{
function ambil_data_pegawai_dttb()
{
    ...
    $orderCol = $columns[$order[0]['column']] ?? 'nama_pegawai';
    $orderDir = $order[0]['dir'] ?? 'asc';

    $this->db->select('id_pegawai,nama_pegawai,tmp_lahir,email,no_hp,nik,nip,alamat');
    $this->db->from('ol_pegawai');

    if (!empty($search['value'])) {
        $this->db->group_start();
        foreach ($columns as $col) {
            if ($col) $this->db->or_like($col, $search['value']);
        }
        $this->db->group_end();
    }
	...
	{
  $columns = [
      'nama_pegawai', 'nip', 'nama_unit', 'nama_komite'
  ];
		$fields = "*,if(status_asesor = 0,'Pegawai',nama_komite) as nama_komite
		";
...
if (!empty($dt_kolom)) {
    foreach ($dt_kolom as $i => $k) {
        if (!empty($k['search']['value'])) {
            $this->db->like($columns[$i], $k['search']['value']);
        }
    }
}
...
/*		$pengurus_barcodepengcab = $this->m_ol_rancak->jumlah_akses_pengurus_barcodepengcab($this->session->id_pegawai,$ids);
	...
	//	$jml = $this->jumlah_akses_pengurus();
	 	$kondisi=array('id_unit'=>$this->session->unit);
		$jml = $this->m_umum->ambil_data_kondisi('ol_pegawai_unit',$kondisi);
