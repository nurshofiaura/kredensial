		...
		if(!empty($cari['value'])) {    //02 Where
		  foreach($dt_kolom as $k){
			if($k['searchable']=='true'){ //cek kalo searchable
				...
				$this->db->where("pembuat_reject", $this->session->refer);
			}
		  }
		}
...
/* 		$kondisi=array('id_level !='=>'99','id_pegawai'=>$id);
	...
	function simpan_reject(){
		$data_pendaftaran = array(
			...
			'pembuat_reject' => $this->session->refer,
...
/* 		$kondisi=array('id_level !='=>'99','id_pegawai'=>$id);
	...
	//	$ids = explode(',', $unit);
	//--------- Ambil nama kolom --------- [coding here] .jpg
		$fields = "*,concat('kV : ',kv,' - mAs : ',mas,' - FPD : ',fpd) as kv";
	//--------- Siapkan Parameter dari datatables ---------
		$draw = intval($this->input->post('draw'));
		$start = intval($this->input->post('start'));
		$length = intval($this->input->post('length'));
	...
	//--------- Mulai Query UTAMA ---------------------------
	    ...
	    $this->db->from('radiologi_fe rf');
			...
			$this->db->join('radiologi_proyeksi rt', 'rt.id_proyeksi=rf.id_proyeksi','left');
...
/* 		$kondisi=array('id_level !='=>'99','id_pegawai'=>$id);
	...
	function simpan_radiologi_fe(){
		...
		$jml_kode = count($id_proyeksi);
		for ($i=0;$i<$jml_kode;$i++){
			if(!empty($kv[$i]) OR $kv[$i] > 0){
			$this->db->select("COUNT(*) as num");
			$this->db->where('id_tindakan',$id_tindakan);
			$this->db->where('id_proyeksi',$id_proyeksi[$i]);
			$q = $this->db->get('radiologi_fe')->row();
			$jml = $q->num;
			if($jml == 0){
				$data_pendaftaran = array(
					'grid' => $grid[$i],
					'id_proyeksi' => $id_proyeksi[$i],
				...
				$this->edit_radiologi_fe($id_proyeksi[$i],$id_tindakan,$id_field_size[$i],$kv[$i],$mas[$i],$fpd[$i],$thickness[$i],$grid[$i]);
			}
		}
		}
	}
	...
	function edit_radiologi_fe($id_proyeksi,$id_tindakan,$id_field_size,$kv,$mas,$fpd,$thickness,$grid){
		$data_pendaftaran = array(
			'grid' => $grid,
			'thickness' => $thickness,
		...
		$this->db->where('id_proyeksi',$id_proyeksi);
		$this->db->where('id_tindakan',$id_tindakan);
	...
	function edit_radiologife(){
		$id_fe = $this->input->post('id_fe');
		$data_pendaftaran = array(
			'grid' => $this->input->post('grid'),
			'thickness' => $this->input->post('thickness'),
			'id_field_size' => $this->input->post('id_field_size'),
...
/* 		$kondisi=array('id_level !='=>'99');
	...
	//	$ids = explode(',', $unit);
		...
		$fields = "*,
			...
			concat('Keluhan : ',keluhan,' - Keterangan : ',ket_pendaftaran_unit) as keluhan,
...
$this->db->where('DATE(tgl_pendaftaran_unit) BETWEEN "'. date('Y-m-d', strtotime($first_date)). '" and "'. date('Y-m-d', strtotime($last_date)).'"');			
	...
	function hasil_all($first_date,$last_date,$key)
	{
	//	$ids = explode(',', $unit);
		$wordsAry = explode("-", $key);
		$wordsCount = count($wordsAry);
		$fields = "*,
			if(waktu_radiologi_result IS NULL,'Belum Baca',DATE_FORMAT(waktu_radiologi_result,'%d-%m-%Y %H:%i:%s')) as waktu_radiologi_result,
			DATE_FORMAT(wkt_daftar_unit,'%d-%m-%Y %H:%i:%s') as wkt_daftar_unit,
			if(waktu_radiologi_result IS NULL,'Belum Baca',timediff(waktu_radiologi_result, wkt_daftar_unit)) as selisih,
			if (p.jk = '1' ,'Laki-laki','Perempuan') as jk,p.nik,p.tmp_lahir,drpmr.nama_pegawai as pj,
			if (p.id_golongan_darah = '0' ,'Belum Ada Data',nama_golongan_darah) as nama_golongan_darah,
			if (p.id_agama = '0' ,'Belum Ada Data',nama_agama) as nama_agama,drpeg.nama_pegawai as internal,
			if (p.id_pekerjaan = '0' ,'Belum Ada Data',nama_pekerjaan) as nama_pekerjaan,
			...
			DATE_FORMAT(p.tgl_lahir,'%d-%m-%Y') as tgl_lahir,DATE_FORMAT(p.dibuat,'%d-%m-%Y %H:%i:%s') as dibuat,peg2.nama_pegawai as petugas,
			pd.id_pendaftaran, pu.id_pendaftaran_unit,if (pd.penunjang = '1' ,'LENGKAP','TIDAK LENGKAP') as v_sign,
			DATE_FORMAT(tgl_pendaftaran_unit,'%d-%m-%Y') as waktu_daftar, u1.nama_unit as order_unit,radiol.nama_pegawai as nama_radiolog,
			if(pd.id_cara_bayar = '6',peg1.nama_pegawai,ka.nama_detil_cara_bayar) as detil_cara_bayar,
			if(pd.id_cara_masuk = '7',u2.nama_unit,if(pd.id_instansi_cara_masuk = '0','-',nama_rujukan_instansi)) as id_instansi_cara_masuk,
			if(pd.id_cara_masuk = '7',rapen.nama_pegawai,if(pd.id_dokter_rujukan = '0','-',nama_rujukan_dokter)) as id_dokter_rujukan,
			concat('Keluhan : ',keluhan,' - Ket Pendaftaran : ',ket_pendaftaran_unit,' - Keterangan Pmr: ',ket_pemeriksaan) as keluhan
...
$this->db->where('DATE(tgl_pendaftaran_unit) BETWEEN "'. date('Y-m-d', strtotime($first_date)). '" and "'. date('Y-m-d', strtotime($last_date)).'"');			
		...
		$this->db->join('ol_pegawai radiol','radiol.id_pegawai=rr.id_radiolog','left');             //04 Form.. left join
...
$this->db->where('DATE(tgl_pendaftaran_unit) BETWEEN "'. date('Y-m-d', strtotime($first_date)). '" and "'. date('Y-m-d', strtotime($last_date)).'"');			
		...
		$this->db->from('radiologi_result rr');
		$this->db->join('ol_pegawai radiol','radiol.id_pegawai=rr.id_radiolog','left');
		$this->db->join('pemeriksaan pmr', 'pmr.barcode_pemeriksaan=rr.barcode_pemeriksaan','left');
		$this->db->join('billing bl', 'bl.barcode_pemeriksaan=pmr.barcode_pemeriksaan','left');
		$this->db->join('tindakan_tarif tt', 'tt.barcode_tindakan_tarif=bl.barcode_tindakan_tarif','left');
		$this->db->join('tindakan t', 't.id_tindakan=tt.id_tindakan','left');
