...
//==================================================== ambil data pegawai
    ...
    public function datatable_pegawai($dt)
    {
        ...
        $orderCol = $this->pegawai_columns[$dt['order'][0]['column']] ?? 'nama_pegawai';
        ...
        // TOTAL
        $total = $this->db->count_all('ol_user');
    ...
    /* ================= CHILD ================= */
    private function child_pegawai_base($pegawai)
    {
        ...
        $this->db->join('ol_validasi v','v.id_kewenangan = ol_logbook.id_kewenangan AND v.barcode_pegawai = ol_pegawai.barcode_pegawai','left',false);
        $this->db->where('ol_pegawai.barcode_pegawai', $pegawai);
        $this->db->where('v.id_kewenangan IS NULL', null, false);
    }
    public function datatable_child($pegawai, $dt)
    {
        // ================= DATA =================
        $this->child_pegawai_base($pegawai);

        if (!empty($dt['search']['value'])) {
            $this->db
                ->group_start()
                ->like('nkr_kewenangan.nama_kewenangan', $dt['search']['value'])
                ->or_like('nkr_kompetensi.nama_kompetensi', $dt['search']['value'])
                ->or_like('kol_sifat_kewenangan.nama_sifat_kewenangan', $dt['search']['value'])
                ->group_end();
        }
...
//==================================================== ambil data logbook
    ...
    private function logbook_base($pegawai)
    {
        $this->db
            ->select("
                    ol_pegawai.barcode_pegawai,ol_pegawai.nama_pegawai,ol_pegawai.id_pegawai,
                    ol_logbook.id_sifat_kewenangan,kol_sifat_kewenangan.nama_sifat_kewenangan,
                    ol_logbook.id_kewenangan,nkr_kewenangan.nama_kewenangan,
                    nkr_kewenangan.id_kompetensi,nkr_kompetensi.nama_kompetensi
            ")
             ->from('ol_logbook')
             ...
             ->join('ol_validasi v','v.id_kewenangan = ol_logbook.id_kewenangan AND v.barcode_pegawai = ol_pegawai.barcode_pegawai','left',false)
             ->where('ol_pegawai.barcode_pegawai', $pegawai)
             ->where('v.id_kewenangan IS NULL', null, false);
    }
    public function datatable_logbook($pegawai,$dt)
    {
        // DATA
        $this->logbook_base($pegawai);
        $this->datatable_search($this->logbook_columns, $dt['search'], $dt['cols']);

        $orderCol = $this->logbook_columns[$dt['order'][0]['column']] ?? 'ol_pegawai.nama_pegawai';
        $orderDir = $dt['order'][0]['dir'] ?? 'asc';
    ...
    public function simpan_rkk($id_kewenangan, $barcode_pegawai, $id_sifat_kewenangan, $status_validasi)
    {
        if (!$id_kewenangan || !$barcode_pegawai || !$id_sifat_kewenangan || !$status_validasi) {
            return [
                'status'  => 'warning',
        ...
        $kondisi_lengkap = array_merge($kondisi_dasar, [
            'id_sifat_kewenangan' => $id_sifat_kewenangan
        ]);

        $sudah_ada = $this->db->where($kondisi_dasar)->count_all_results('ol_validasi');
        $sudah_sama = $this->db->where($kondisi_lengkap)->count_all_results('ol_validasi');

        // ================= INSERT =================
        if ($sudah_ada == 0) {

            $insert = [
                'id_validasi'          => $this->m_rancak->kode_generator_urut(15, 'RV'),
            ...
            if ($this->db->insert('ol_validasi', $insert)) {
                return [
                    'status'  => 'success',
                    'title'   => 'Berhasil',
                    'message' => 'Data RKK berhasil disimpan',
                    'reload'   => true
                ];
            }
        ...
        if ($sudah_sama == 0) {
            ...
            if ($this->db->affected_rows()) {
                return [
                    'status'  => 'success',
                    'title'   => 'Berhasil',
                    'message' => 'Data RKK berhasil diperbarui'
                ];
            }
    ...
    public function detail_pegawai($id)
    {
        return $this->db
            ->select("
                ...
                ol_pegawai.nip,
                ol_unit.nama_unit,
                IF(ol_user.status_asesor=0,'Pegawai',kol_komite.nama_komite) AS status_label
            ")
            ->from('ol_user')
            ->join('ol_pegawai','ol_pegawai.id_pegawai=ol_user.id_pegawai','left')
            ->join('ol_pegawai_unit','ol_pegawai_unit.id_pegawai=ol_pegawai.id_pegawai','left')
            ->join('ol_unit','ol_unit.id_unit=ol_pegawai_unit.id_unit','left')
            ->join('kol_komite','kol_komite.id_komite=ol_user.status_asesor','left')
            ->where('ol_pegawai.id_pegawai', $id)
            ->get()
            ->row_array();
    }
